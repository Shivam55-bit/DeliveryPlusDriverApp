...
export default function CompleteJobScreen({ navigation, route }) {
  ...
  const pricingSummary = getJobPricingSummary(currentJob, currentJob?.myAssignment);

  const initialAmount =
    pricingSummary.canShowPrice && pricingSummary.amountToCollect > 0
      ? pricingSummary.amountToCollect.toFixed(2)
      : "";
    ...
    <SafeAreaView style={styles.safe} edges={["top", "left", "right"]}>
      ...
      >
        ...
        <View style={styles.card}>
          ...
          {/* Amount to Collect Summary Card */}
          {pricingSummary.canShowPrice ? (
            <View style={styles.amountToCollectBox}>
              <Text style={styles.amountToCollectLabel}>AMOUNT TO COLLECT</Text>
              <Text style={styles.amountToCollectValue}>
                {pricingSummary.formattedAmountToCollect}
              </Text>
              <Text style={styles.amountToCollectSubtext}>
                Price provided by Delivery Plus
              </Text>

              {/* Line item breakdown for hourly / detailed jobs */}
              {pricingSummary.isHourly ? (
                <View style={styles.breakdownWrap}>
                  <View style={styles.breakdownDivider} />
                  <View style={styles.breakdownItemRow}>
                    <Text style={styles.breakdownItemLabel}>
                      Base Cost ({pricingSummary.formattedBaseDuration})
                    </Text>
                    <Text style={styles.breakdownItemVal}>
                      {pricingSummary.formattedBaseAmount}
                    </Text>
                  </View>
                  ...
                  {pricingSummary.stairsFee > 0 ? (
                    <View style={styles.breakdownItemRow}>
                      <Text style={styles.breakdownItemLabel}>Stairs Fee</Text>
                      <Text style={styles.breakdownItemVal}>
                        {pricingSummary.formattedStairsFee}
                      </Text>
                    </View>
                  ) : null}

                  {pricingSummary.travelBackFee > 0 ? (
                    <View style={styles.breakdownItemRow}>
                      <Text style={styles.breakdownItemLabel}>Travel Back</Text>
                      <Text style={styles.breakdownItemVal}>
                        {pricingSummary.formattedTravelBackFee}
                      </Text>
                    </View>
                  ) : null}

                  {pricingSummary.extraCharges > 0 ? (
                    <View style={styles.breakdownItemRow}>
                      <Text style={styles.breakdownItemLabel}>Extra Charges</Text>
                      <Text style={styles.breakdownItemVal}>
                        {pricingSummary.formattedExtraCharges}
                      </Text>
                    </View>
                  ) : null}
                  ...
                  <View style={styles.breakdownItemRow}>
                    ...
                    <Text style={styles.breakdownTotalVal}>
                      {pricingSummary.formattedFinalTotal}
                    </Text>
                  </View>
                </View>
              ) : null}
